...
|主域名|访问集群实例时若使用域名访问则填写此项，会向华为云以该域名添加子域名的记录集解析到实例的IP，若不填写则通过IP访问|     GameFlexMatch.com    |
...
需要通过域名访问集群实例时，可在创建fleet时添加主域名

### 操作步骤

1. 准备一个在华为云备案的域名作为主域名，例如”gameflexmatch.com"

2. 创建fleet时主域名参数填写"gameflexmatch.com"

3. 等待fleet创建完成；

   - 创建实例时，会根据主域名生成一个子域名，格式为“**{随机字符串}.{主域名}**”，如“abcdefg.gameflexmatch.com"，然后将子域名添加到DNS的A类解析记录集中，建立”**域名-IP**“的映射关系，从而能通过域名访问到对应的实例；

   - 当域名能成功解析后，实例将拉起进程；

4. fleet创建完成

5. 当fleet缩容和删除时，对应的解析记录也会相应删除；



注意：

1. 域名需要在华为云备案，否则添加记录后可能无法解析成功，fleet将不会创建成功；
2. 域名解析记录有上限，需要结合实际配额设置fleet的最大实例数。
