...
1.	租户，表示使用华为云服务的企业用户
2.	用户，表示租户应用的使用者
3.	GameFlexMatch服务，表示华为云提供的应用托管服务平台
4.	租户管理面，表示租户与GameFlexMatch服务交互的应用，比如游戏大厅
5.	租户托管应用，表示租户托管在GameFlexMatch服务平台上的应用，比如游戏应用
...
```
6. 租户管理面通过CreateClientSession接口给用户获取连接信息，用户接入使用client session接入后，托管盈余公进程会调用AcceptClientSession去通知GameFlexMatch平台当前client session已经接入，GameFlexMatch平台会设置改client session状态为active；如果client session创建60秒后都没有接入的话，状态会转变为timeout，该client session不可再用
...
```
7. 在用户断开连接后，托管应用进程需要调用RemoveClientSession API 去移除该用户，GameFlexMatch平台会将相应的client session设置为"complete"状态，并回收该配额
