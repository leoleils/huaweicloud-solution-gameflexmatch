...
function start_service(){
	...
	./appgateway-${version} -gateway-addrress $GATEWAY_ADDR -database-address $DATABASE_ADDR -database-name $DATABASE_NAME -database-user-name $DATABASE_USER -database-password $DATABASE_PASSWORD --aass-address $AASS_ADDR -influx-address $INFLUX_ADDR -influx-username $INFLUX_USER -influx-password $INFLUX_PASSWORD -influx-dbname $INFLUX_DBNAME
}
