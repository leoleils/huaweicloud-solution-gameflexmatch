...
export DEFAULT_GROUP_MAX_SIZE=1							# 默认的弹性伸缩组最大数量
export DEFAULT_GROUP_MIN_SIZE=1							# 默认的弹性伸缩组最小数量
export DEFAULT_GROUP_DESIRED_SIZE						# 默认的弹性伸缩最最小数量
...
export IMAGE_VPC_NAME={image_vpc_name} # 用于镜像打包的ECS绑定的VPC名
export IMAGE_SUBNET_NAME={image_subnet_name} # 用于镜像打包的ECS绑定的子网名
export DEFAULT_IMAGE_REF="CentOS 7.2 64bit" # 用于镜像打包的ECS默认操作系统
export DEFAULT_SCRIPT_PATH=metaspace/image_env.sh # 镜像环境配置脚本 OBS路径
export DEFAULT_AUXPROXY_PATH=metaspace/auxproxy.zip # AuxProxy组件压缩包 OBS路径 用于镜像环境配置
export IMAGE_DISK_SIZE=40
export LTS_AGENCY_NAME=lts_ecm_trust # LTS授权委托名称 用于ECS安装ICAgent
...
export LOG_ROTATE_SIZE=100								# 每个log文件的尺寸(MB)
export LOG_BACKUP_COUNT=100								# 每个日志文件储存的最大数量
export LOG_MAX_AGE=7									# 每个日志存储的最大时间					
export MAX_PROCESS_NUM_PER_FLEET=50						# 每个Fleet的进程数量最大限制
